...
public abstract class OrganPart extends Actor implements Connectable {
    ...
    private void drawSuturePoints(Batch batch){
        for(SuturePoint sp : suturePoints){
            ...
            float w = com.suturesapp.workspace.utils.Constants.SMALLINTESTINE_SCALE * (sprite.getTexture().getWidth())/2f *someScale;
            float h = com.suturesapp.workspace.utils.Constants.SMALLINTESTINE_SCALE * (sprite.getTexture().getHeight())/2f *someScale;
    ...
    public float getVerticesScale(){
        return getHeight()/ origin.y / com.suturesapp.workspace.utils.Constants.VERTICES_SCALE_FACTOR;
    }
