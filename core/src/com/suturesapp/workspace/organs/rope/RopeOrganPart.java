 ...
 */
public abstract class RopeOrganPart extends com.suturesapp.workspace.organs.OrganPart implements com.suturesapp.workspace.organs.Openable {
    ...
    public RopeOrganPart(World world, OrthographicCamera camera, com.suturesapp.workspace.organs.Organ callback, int id, float scale, Vector2 position, float rotation) {
        super(world, camera, callback, "", scale, position, rotation);
        this.id = id;
    }

    public RopeOrganPart(com.suturesapp.workspace.organs.OrganPartDefinition opDef) {
        super(opDef);
...
//    }
    ...
    public boolean close(float x, float y) {
        com.suturesapp.workspace.organs.OpenableSide side = getTheOpenOs();
        if(side != null) {
            side.close();
            return true;
        }
    ...
    public boolean hasAnOpenSide(){
        try {
            return openableSides.get(0).state== com.suturesapp.workspace.organs.OpenableSide.State.OPEN
                    || openableSides.get(1).state== com.suturesapp.workspace.organs.OpenableSide.State.OPEN ;
        }catch (NullPointerException npe){
            return false;
        }
    }
    ...
    public com.suturesapp.workspace.organs.OpenableSide getTheOpenOs(){
        for(com.suturesapp.workspace.organs.OpenableSide os: openableSides){
            if(os.state== com.suturesapp.workspace.organs.OpenableSide.State.OPEN)
                return os;
        }
        return null;
    }
    ...
    public com.suturesapp.workspace.organs.SuturePoint getTheNoXYSp(){
        for(com.suturesapp.workspace.organs.SuturePoint sp: suturePoints){
            // neither on X nor Y axes
            if(sp.getLocalCoord().x!=0 && sp.getLocalCoord().y!=0)
                return sp;
