...
public abstract class Tool extends InputAdapter implements GestureDetector.GestureListener {
    ...
    void extractHitBody(){
        ...
        if(com.suturesapp.workspace.utils.Constants.VERBOSE_GESTURE){
            ...
            if(hitBody!=null)
                Gdx.app.log("extractHitBody", "Hit: "+hitBody.getUserData().getClass().getSimpleName()
                        +( (com.suturesapp.workspace.organs.OrganPart) hitBody.getUserData() ).getIdentifier()
                        + ((hitBody.getUserData() instanceof com.suturesapp.workspace.organs.rope.RopeOrganPart)?
                        ( (com.suturesapp.workspace.organs.rope.RopeOrganPart) hitBody.getUserData() ).id:"")
                        +" at "+((com.suturesapp.workspace.organs.OrganPart)hitBody.getUserData()).body.getLocalPoint(new Vector2(testPoint.x, testPoint.y))
                        +" equivalent to json coordinates: "
                        +((com.suturesapp.workspace.organs.OrganPart)hitBody.getUserData()).jsonCoordinatesFromVertex(
                        ((com.suturesapp.workspace.organs.OrganPart)hitBody.getUserData()).body.getLocalPoint(new Vector2(testPoint.x, testPoint.y))
                ).toString());
        }
    }
    ...
    public boolean bodyMoving() throws NullPointerException{
        if (hitBody != null) {
            ...
            def.maxForce = 1000.0f * com.suturesapp.workspace.organs.rope.SmallIntestine.getLENGTH() * hitBody.getMass();
    ...
    public boolean zoom(float initialDistance, float distance) {
        ...
        camera.zoom = com.suturesapp.workspace.utils.Utils.zoomToFitInterval((initialDistance / distance) * currentZoom);
        camera.update();
        return true;
    }
