package com.suturesapp.smallintestinedemo.organs.rectum;
 ...
 */
public class Rectum extends com.suturesapp.smallintestinedemo.organs.Organ {
    ...
    public Rectum(World world, OrthographicCamera camera) {
        ...
        for(int i=1; i<=SIZE; i++){
            com.suturesapp.smallintestinedemo.organs.OrganPart part = new RectumOrganPart(world, camera, this, ""+i, SCALE, getOrganPartPosition(i),0);
            organParts.put(part.getIdentifier(), part);
            addActor(part);
        }

        JSONArray joints = com.suturesapp.smallintestinedemo.utils.Utils.loadJoints("rectum_initial_joints.json");
    ...
    public void loadState(Kryo kryo, Input input){
        ...
        for(int i=1; i<=SIZE; i++){
            com.suturesapp.smallintestinedemo.organs.OrganPart op = kryo.readObject(input, RectumOrganPart.class);
