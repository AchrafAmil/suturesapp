...
public abstract class OrganPart extends Actor implements Connectable {
    ...
    public com.suturesapp.smallintestinedemo.organs.Organ organCallback ;
    ...
    protected ArrayList<com.suturesapp.smallintestinedemo.organs.SuturePoint> suturePoints = new ArrayList<>();

    protected ArrayList<com.suturesapp.smallintestinedemo.organs.OpenableSide> openableSides = new ArrayList<>();

    protected List<com.suturesapp.smallintestinedemo.organs.OpenableSide.State> openableSidesStatesBuffer;


    public OrganPart(World world, OrthographicCamera camera, com.suturesapp.smallintestinedemo.organs.Organ callback, String identifier, float scale, Vector2 position, float rotation ){
        this.mWorld = world;
        this.camera = camera;
    ...
    public OrganPart(World world, OrthographicCamera camera, com.suturesapp.smallintestinedemo.organs.Organ callback, float scale, com.suturesapp.smallintestinedemo.organs.OrganPartDefinition opDef){
    ...
    public OrganPart(com.suturesapp.smallintestinedemo.organs.OrganPartDefinition opDef){
        this(com.suturesapp.smallintestinedemo.organs.OrgansHolder.world, com.suturesapp.smallintestinedemo.organs.OrgansHolder.camera, com.suturesapp.smallintestinedemo.organs.OrgansHolder.organFromName(opDef.getOrganName()),
                com.suturesapp.smallintestinedemo.utils.Utils.scaleFromName(opDef.getOrganName()), opDef );
    }
    ...
    public boolean loadBufferedOpenableSides(){
        ...
        for(com.suturesapp.smallintestinedemo.organs.OpenableSide.State state: openableSidesStatesBuffer){
            com.suturesapp.smallintestinedemo.organs.OpenableSide side = openableSides.get(openableSidesStatesBuffer.indexOf(state));
            side.setState(state);
        }
        return true;
    }
    ...
    protected void setupBody(String path){
        ...
        if(this.getClass().getSimpleName().equals("RectumOrganPart") && this.identifier.equals("6"))
            bDef.type = BodyDef.BodyType.KinematicBody;         // just while developing
        if(this.getClass().getSimpleName().equals("ColonOrganPart") && ((com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart)this).id==118)
            bDef.type = BodyDef.BodyType.KinematicBody;         // just while developing
        if(this.getClass().getSimpleName().equals("ColonOrganPart") && ((com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart)this).id==57)
            bDef.type = BodyDef.BodyType.KinematicBody;         // just while developing
        ...
        fix.density = 0.1f;
        if(this instanceof com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart)
            fix.density = 0.5f;
        fix.friction = 0f;
    ...
    private void drawBaseSprite(Batch batch){
        ...
        bodyPixelPos = camera.project(tmpVec.set(
                body.getPosition().x, body.getPosition().y, 0).add(com.suturesapp.smallintestinedemo.utils.Utils.cameraPosition(camera)))
    ...
    private void drawOpenableSides(Batch batch){
        for(com.suturesapp.smallintestinedemo.organs.OpenableSide side: openableSides){
    ...
    private void drawSuturePoints(Batch batch){
        for(com.suturesapp.smallintestinedemo.organs.SuturePoint sp : suturePoints){
            ...
            spPixelPos = camera.project(tmpVec.set(spWorldCoord.x, spWorldCoord.y, 0)
                    .add(com.suturesapp.smallintestinedemo.utils.Utils.cameraPosition(camera)))
    ...
    @Override
    public boolean addSuturePoint(com.suturesapp.smallintestinedemo.organs.SuturePoint sp) {
    ...
    @Override
    public boolean deleteSuturePoint(com.suturesapp.smallintestinedemo.organs.SuturePoint sp) {
    ...
    public boolean deleteSuturePoint(RevoluteJoint joint) {
        for(com.suturesapp.smallintestinedemo.organs.SuturePoint sp: this.suturePoints){
    ...
    @Override
    public ArrayList<com.suturesapp.smallintestinedemo.organs.SuturePoint> getSuturePoints() {
     ...
     */
    @Override
    public com.suturesapp.smallintestinedemo.organs.SuturePoint correctAnchorAndRotation(com.suturesapp.smallintestinedemo.organs.SuturePoint suturePoint) {
        return suturePoint;
    }
     ...
     */
    public com.suturesapp.smallintestinedemo.organs.OrganPartDefinition getOPDef(){
        com.suturesapp.smallintestinedemo.organs.OrganPartDefinition opDef = new com.suturesapp.smallintestinedemo.organs.OrganPartDefinition();
        ...
        if(this instanceof com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart)
            opDef.ropeId = ((com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart)this).id;

        for(com.suturesapp.smallintestinedemo.organs.OpenableSide os : openableSides){
    ...
    public Vector2 pushToEdge(Vector2 vec){
        ...
        for(float i=0; i<200; i+=step){
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec1.x+=step, vec1.y)){
                //vec1.x-=step;
                return this.body.getLocalPoint(vec1).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec2.x-=step, vec2.y)){
                //vec2.x+=step;
                return this.body.getLocalPoint(vec2).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec3.x, vec3.y+=step)){
                //vec3.y-=step;
                return this.body.getLocalPoint(vec3).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec4.x, vec4.y-=step)){
                //vec4.y+=step;
                return this.body.getLocalPoint(vec4).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec5.x+=step, vec5.y+=step)){

                return this.body.getLocalPoint(vec5).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec6.x-=step, vec6.y-=step)){

                return this.body.getLocalPoint(vec6).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec7.x-=step, vec7.y+=step)){

                return this.body.getLocalPoint(vec7).cpy();
            }
            if(!com.suturesapp.smallintestinedemo.utils.Utils.fixturesContains(fixtures,vec8.x+=step, vec8.y-=step)){
