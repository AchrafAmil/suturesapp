        ...
        if(Constants.VERBOSE_GESTURE){
            ...
            if(hitBody!=null)
                Gdx.app.log("GESTURE", "Hit: "+hitBody.getUserData().getClass().getSimpleName()
                        +( (com.suturesapp.smallintestinedemo.organs.OrganPart) hitBody.getUserData() ).getIdentifier()
                        + ((hitBody.getUserData() instanceof com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart)?
                        ( (com.suturesapp.smallintestinedemo.organs.rope.RopeOrganPart) hitBody.getUserData() ).id:"")
                        +" at "+((com.suturesapp.smallintestinedemo.organs.OrganPart)hitBody.getUserData()).body.getLocalPoint(new Vector2(testPoint.x, testPoint.y)));
        }
    }
